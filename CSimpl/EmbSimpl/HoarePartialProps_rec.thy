...
next
  ...
  qed
next
  case (CallRec1 p \<Theta> F P Q A) 
  have valid_c: "\<And>n. \<Gamma>,\<Theta> \<Turnstile>n:\<^bsub>/F\<^esub> (P)  (the (\<Gamma> p)) Q,A" by fact
  have p_dom:"p \<in> dom \<Gamma>" by fact
  show ?case
  proof (rule cnvalidI)
    fix s t
    assume ctxt:"\<forall>(P, p, Q, A)\<in>\<Theta>. \<Gamma>\<Turnstile>n:\<^bsub>/F\<^esub> P Call p Q,A" and
           exec:"\<Gamma>\<turnstile> \<langle>Call p,Normal s\<rangle> =n\<Rightarrow> t" and
           P:"s \<in> P" and
           t_notin_F:"t \<notin> Fault ` F "
    from ctxt exec have "t \<in> Normal ` Q \<union> Abrupt ` A"
    proof (cases n)
      case 0 
      assume a0:"\<forall>(P, p, Q, A)\<in>\<Theta>. \<Gamma>\<Turnstile>n:\<^bsub>/F\<^esub> P Call p Q,A"  and
             a1:"\<Gamma>\<turnstile> \<langle>Call p,Normal s\<rangle> =n\<Rightarrow> t" and 
             a2:"n = 0" 
      thus ?thesis by (fastforce elim!: execn_elim_cases)
    next
      case (Suc m)
      assume a0:"\<forall>(P, p, Q, A)\<in>\<Theta>. \<Gamma>\<Turnstile>n:\<^bsub>/F\<^esub> P Call p Q,A" and
             a1:"\<Gamma>\<turnstile> \<langle>Call p,Normal s\<rangle> =n\<Rightarrow> t" and
             a2:"n = Suc m "
      then have env_sucn:"\<forall>(P, p, Q, A)\<in>\<Theta>. \<Gamma>\<Turnstile>Suc m:\<^bsub>/F\<^esub> P Call p Q,A" by auto
      hence env_n:"\<forall>(P, p, Q, A)\<in>\<Theta>. \<Gamma>\<Turnstile>m:\<^bsub>/F\<^esub> P Call p Q,A"
        unfolding nvalid_def
        by (fastforce  intro: execn_Suc)
      have step_sucn:"\<Gamma>\<turnstile> \<langle>Call p,Normal s\<rangle> =Suc m\<Rightarrow> t" using a2 a1 by auto
      then have "\<Gamma>\<turnstile> \<langle>the (\<Gamma> p),Normal s\<rangle> =m\<Rightarrow> t"    
        using execn_Normal_elim_cases(11) p_dom by fastforce       
      also from valid_c have "\<Gamma>,\<Theta>\<Turnstile>m:\<^bsub>/F\<^esub> P the (\<Gamma> p) Q,A" by fastforce
      ultimately show ?thesis using env_n P t_notin_F unfolding cnvalid_def nvalid_def
        by fastforce      
    qed
    then show " t \<in> Normal ` Q \<union> Abrupt ` A" by auto
  qed   
next
...
theorem hoare_sound': "\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> P c Q,A \<and> \<Theta>={} \<Longrightarrow> \<Gamma>\<Turnstile>\<^bsub>/F\<^esub> P c Q,A"
proof-
  assume a0:"\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> P c Q,A \<and> \<Theta>={}"
  then have valid:"\<Gamma>,\<Theta>\<Turnstile>\<^bsub>/F\<^esub> P c Q,A" using hoare_sound by fastforce
  then have "(\<forall>(P,p,Q,A)\<in>\<Theta>. \<Gamma>\<Turnstile>\<^bsub>/F\<^esub> P (Language.Call p) Q,A)" using a0 by auto
  then show ?thesis using valid unfolding cvalid_def by fastforce
qed
