...
(*lemma "Sta p R  \<Longrightarrow> (x,y)\<in>R \<Longrightarrow> x=Normal x' \<Longrightarrow> y = Normal y' \<Longrightarrow> x'\<in>p \<Longrightarrow>
       y' \<in> p " 
proof -
assume a0: "Sta p R" and
       a1: "(x,y) \<in>R" and
       a2: "x=Normal x'" and
       a3: "y = Normal y'" and
       a4: "x' \<in> p"
thus ?thesis  unfolding Sta_def by auto
qed
*)
...
proof -
        ...
        a' \<subseteq> a"
  {
    ...
    proof -
      ...
      have "(snd(l!i), snd(l!(Suc i))) \<in> 
             (case_prod (\<lambda>x y. (Normal x, Normal y))) ` G" 
      proof -
        have "(snd(l!i), snd(l!(Suc i))) \<in> 
             (case_prod (\<lambda>x y. (Normal x, Normal y))) ` G'"
        using comm_dest1 [OF a0 a a00 a11 a22]  by auto
  ...
  proof -
     ...
     proof -
       {fix i ns ns'
        ...
        \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` G"         
        proof (cases i)
          case 0 
        ...
        next
          ...
          proof -
            ...
            hence "(snd (((P, t) # xs) ! n), snd (((P, t) # xs) ! Suc n)) \<in> (\<lambda>(pa, p). (Normal pa, Normal p)) ` G"
        ...
        next
          ...
          proof -
            ...
            have "snd (snd (\<Gamma>, (Q, Normal s) # xs) ! Suc n) \<notin> Fault ` F \<and> snd (\<Gamma>, (Q, Normal s) # xs) = (Q, Normal s) # xs"
              using Suc a13 by force
            hence "(snd (((Q, Normal s) # xs) ! n), snd (((Q, Normal s) # xs) ! Suc n)) \<in> (\<lambda>(pa, p). (Normal pa, Normal p)) ` G"
...
lemma comm_union:
 assumes 
   ...
   a2: "xs\<noteq>[] \<and> ys\<noteq>[] \<and> snd (last xs) = Normal s \<and> snd (ys!0) = Normal t" and
   a3: "(s,t) \<in> G"
   ...
   proof -
     {fix i ns ns'
      ...
      proof (cases "Suc i>length xs")
        ...
        moreover have "snd (ys!(i-(length xs))) = Normal ns \<and> 
                       snd (ys!((Suc i)-(length xs))) = Normal ns'"
          using a13 all_ys True by fastforce
        ...
        ultimately have "(snd(ys!(i-(length xs))), snd(ys!Suc (i-(length xs)))) \<in> 
                          (case_prod (\<lambda>x y. (Normal x, Normal y))) ` G" 
        using a1 comm_dest1[of \<Gamma> ys G q' a' F "i-length xs" ns ns'] True Suc_diff_le by fastforce 
      ...
      next
        ...
        proof (cases "Suc i < length xs")
          ...
          proof -
            have "0 < length xs"
              by (meson a2 length_greater_0_conv)
            then have "(xs @ ys) ! (length xs - 1) = last xs"
              by (metis (no_types) One_nat_def a2 all_xs diff_less last_conv_nth less_Suc0)
            then show ?thesis
              using a2 by force
          qed 
          then have "snd (last xs) \<notin> Fault ` F" using all_xs a2 by auto          
          moreover have "\<Gamma>\<turnstile>\<^sub>c (xs ! i) \<rightarrow> ( xs ! Suc i)" 
            using True all_xs a12 by fastforce
          moreover have "snd ((snd (\<Gamma>,xs))!i) = Normal ns \<and> 
                         snd ((snd (\<Gamma>,xs))!(Suc i)) = Normal ns'"
            using True all_xs a13 by fastforce
          ultimately have"(snd(xs!i), snd(xs!(Suc i))) \<in> 
             (case_prod (\<lambda>x y. (Normal x, Normal y))) ` G"
            using a0 comm_dest1[of \<Gamma> xs G q a F i ns ns'] True by fastforce
        ...
        next
          ...
          then have "snd ((xs@ys)!i) = Normal s" 
            using a2 all_xs  suc_i last_conv_nth lessI nth_append by fastforce 
          also have "snd ((xs@ys)!(Suc i)) = Normal t"
            using a2 all_ys suc_i by fastforce
          ultimately show ?thesis using a3 unfolding Satis_def by fastforce
        qed
      qed
     } thus ?thesis by auto          
   qed
  ...
  "par_assum \<equiv> 
     \<lambda>(pre, rely) F. {c. 
       ...
       (fst c)\<turnstile>\<^sub>p((snd c)!i)  \<rightarrow>\<^sub>e ((snd c)!(Suc i)) \<longrightarrow>        
         (snd((snd c)!i), snd((snd c)!(Suc i))) \<in> 
           (case_prod (\<lambda>x y. (Normal x, Normal y))) ` rely)}"
     ...
     {c. snd (last (snd c)) \<notin> Fault ` F \<longrightarrow> 
         (\<forall>i ns ns'. 
            ...
            snd ((snd c)!i) = Normal ns \<and> snd ((snd c)!(Suc i)) = Normal ns' \<longrightarrow>            
              (snd((snd c)!i), snd((snd c)!(Suc i))) \<in> 
                 (case_prod (\<lambda>x y. (Normal x, Normal y))) ` guar) \<and> 
...
|Spec: "\<lbrakk>p \<subseteq> {s. (\<forall>t. (s,t)\<in>r \<longrightarrow> t \<in> q) \<and> (\<exists>t. (s,t) \<in> r)};        
        (\<forall>s t. s\<in>p \<and> t\<in>q \<longrightarrow> (s,t) \<in> G);
...
| While: "\<lbrakk> \<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c sat [p \<inter> b, R, G, p,a];            
            Sta p R; Sta (p \<inter> (-b)) R; Sta a R; (\<forall>s. (s,s) \<in> G)\<rbrakk> \<Longrightarrow>
...
| Seq: "\<lbrakk> \<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c1 sat [p, R, G, q,a]; \<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c2 sat [q, R, G, r,a];         
         Sta a R; (\<forall>s. (s,s) \<in> G)\<rbrakk> \<Longrightarrow>
        \<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (Seq c1 c2) sat [p, R, G, r,a]"

| Await: "\<lbrakk> \<Gamma>\<^sub>\<not>\<^sub>a,{}\<turnstile>\<^bsub>/F\<^esub> (p \<inter> b) c q,a;
           (\<forall>s t. s\<in>p \<and> t\<in>(q \<union> a)\<longrightarrow> (s,t) \<in> G);                      
...
| Guard: "\<lbrakk> \<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c sat [p \<inter> g, R, G, q,a];           
        Sta (p \<inter> g) R; (\<forall>s. (s,s) \<in> G)\<rbrakk> \<Longrightarrow>
...
| Guarantee:  "\<lbrakk> \<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c sat [p \<inter> g, R, G, q,a];
                Sta p R;f\<in>F; (\<forall>s. (s,s) \<in> G) \<rbrakk> \<Longrightarrow>
...
| DynCom: "(\<forall>s \<in> p. (\<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (c s) sat [p, R, G, q,a])) \<Longrightarrow>    
          (Sta p R) \<and> (Sta q R) \<and> (Sta a R)  \<Longrightarrow> (\<forall>s. (s,s) \<in> G) \<Longrightarrow>
            \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (DynCom c) sat [p, R, G, q,a]"

| Throw: "\<lbrakk>Sta a R; (\<forall>s. (s,s) \<in> G) \<rbrakk> \<Longrightarrow>
...
| Catch: "\<lbrakk> \<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c1 sat [p, R, G, q,r]; 
           ...
           Sta q R; (\<forall>s. (s,s) \<in> G)\<rbrakk> \<Longrightarrow>
...
definition Abr ::  "('s,'p,'f) rgformula \<Rightarrow> ('s set)" where
  "Abr x \<equiv> fst(snd(snd(snd(snd x))))"

definition Rely :: "('s,'p,'f) rgformula \<Rightarrow> ('s tran) set" where
  "Rely x \<equiv> fst(snd(snd x))"

definition Guar ::  "('s,'p,'f) rgformula \<Rightarrow> ('s tran) set" where
...
lemma stable_q_r_q: 
  assumes a0:"Sta q R"  and       
          ...
          a2:"(snd(l!i), snd(l!(Suc i))) \<in> 
             (case_prod (\<lambda>x y. (Normal x, Normal y))) ` R"
...
lemma Skip_sound: 
  "Sta q R \<Longrightarrow>       
   (\<forall>s. (s,s) \<in> G)  \<Longrightarrow>
   \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> Skip sat [q,R, G, q,a]"
proof -  
 assume
    a0:"Sta q R" and    
    a1:"(\<forall>s. (s,s) \<in> G)"
  {
    fix s
      ...
      proof - 
      {
       ...
       { fix i
         ...
         ultimately have "fst (l ! i) = Throw \<and> (\<exists>s2. snd (l ! i) = Normal s2)"      
             using cp assum env_tran asuci zero_throw_all_throw by metis
         then have "\<not> (\<Gamma>1\<turnstile>\<^sub>c(l!i)  \<rightarrow> (l!(Suc i)))"
...
lemma Basic_sound: 
       ...
       \<forall>s t. s\<in>p \<and> t\<in>q \<longrightarrow> (s,t) \<in> G \<Longrightarrow>       
...
proof -  
 assume
    ...
    a1:"\<forall>s t. s\<in>p \<and> t\<in>q \<longrightarrow> (s,t) \<in> G" and
    a2:"Sta p R" and
    a3:"Sta q R" 
{
    fix s
    ...
    {   
      ...
      proof - 
      {
       ...
       { 
         ...
         using  last_l len_l cp env_tran_right a3 suc_k_skip_q assum k_comp_tran stability [of q R l "Suc k" "((length l) - 1)" _ \<Gamma>] 
           by fastforce                 
       } thus ?thesis by auto qed
       note res = conjI [OF concl concr]               
      }              
    ...
    {   
      ...
      proof - 
      {
       ...
       { fix k
         ...
         qed                                                      
         obtain s' s'' where ss:"sk = Normal s' \<and> s' \<in> (p) \<and>  ssk = Normal s'' \<and> s'' \<in> q"
           using suc_k_skip_q k_basic by fastforce
         then have "(snd(l!k), snd(l!(Suc k))) \<in> 
                   (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)"
           using a1 a2 tran_pair by force
       } thus ?thesis by auto qed
...
lemma Await_sound: 
       "\<Gamma>\<^sub>\<not>\<^sub>a,{}\<turnstile>\<^bsub>/F\<^esub> (p \<inter> b) e q,(a) \<Longrightarrow>
       (\<forall>s t. s\<in>p \<and> t\<in>(q \<union> a)\<longrightarrow> (s,t) \<in> G) \<Longrightarrow>
...
proof -  
 assume
    a0: "\<Gamma>\<^sub>\<not>\<^sub>a,{}\<turnstile>\<^bsub>/F\<^esub> (p \<inter> b) e q,(a)" and
    a1:"(\<forall>s t. s\<in>p \<and> t\<in>(q \<union> a)\<longrightarrow> (s,t) \<in> G)" and
    a2:"Sta p R" and
    a3:"Sta q R" and
    a4:"Sta a R" 
{
    ...
    {   
      ...
      proof - 
      {
       ...
       { fix k ns ns'
         ...
         then have before_k_all_evn:"\<forall>j. 0\<le>j \<and> j < k  \<longrightarrow> (\<Gamma>\<turnstile>\<^sub>c(l!j)  \<rightarrow>\<^sub>e (l!(Suc j)))"
               using a00 a11  by fastforce
         then have k_basic:"ck = Await b e \<and> sk \<in> Normal ` (p)" 
           using  cp env_tran_right a2 assum a00 a11 a22 stability[of p R l 0 k k \<Gamma>] tran_pair
           by force
         have suc_skip: "csk = Skip  \<or> (csk = Throw \<and> (\<exists>s1. ssk = Normal s1))"
            using a0 a00 k_basic tran_pair await_skip by blast
         have suc_k_skip_q:"ssk \<in> Normal ` q \<or> ssk \<in> Normal ` (a)"           
         proof -         
           obtain s' where k_s: "sk =Normal s' \<and> s' \<in> (p)" 
             using k_basic by auto
           then obtain t where ssk_normal:"ssk = Normal t" 
           using  k_basic tran_pair a0 a22  by auto               
           have "\<Gamma>\<^sub>\<not>\<^sub>a,{}\<Turnstile>\<^bsub>/F\<^esub> (p \<inter> b) e q,(a)"
           using  a0 hoare_sound 
             by fastforce
           then have e_auto:"\<Gamma>\<^sub>\<not>\<^sub>a,{}\<Turnstile>\<^bsub>/F\<^esub> (p \<inter> b) e q,(a)" 
...
proof -  
  ...
  have a03:"snd (last (l1)) \<notin> Fault ` F  \<longrightarrow>(\<forall>i ns ns'.                 
               ...
               snd ((snd (\<Gamma>, l1))!i) = Normal ns \<and> snd ((snd (\<Gamma>, l1))!(Suc i)) = Normal ns' \<longrightarrow>                               
                 (snd((snd (\<Gamma>, l1))!i), snd((snd (\<Gamma>, l1))!(Suc i))) \<in> 
                    (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)) \<and> 
     ...
     using a03 a1 a2 by force    
      ...
      then have " \<Gamma>\<turnstile>\<^sub>c(l!k)  \<rightarrow> (l!(Suc k)) \<longrightarrow> 
        snd (l!k) = Normal ns \<and> snd (l!(Suc k)) = Normal ns'\<longrightarrow>(snd (l ! k), snd (l ! Suc k)) \<in> 
            (\<lambda>(x, y). (Normal x, Normal y)) ` (G) " 
        using l1_l last_l by auto
    } then have l_side:"Suc k < length l \<longrightarrow>
    \<Gamma>\<turnstile>\<^sub>c l ! k \<rightarrow> l ! Suc k \<longrightarrow>
    snd (l ! k) = Normal ns \<and> snd (l ! Suc k) = Normal ns' \<longrightarrow>
    (snd (l ! k), snd (l ! Suc k))
    \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` (G)" by auto
...
lemma If_sound: 
      "\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c1 sat [p \<inter> b,  R, G, q,a] \<Longrightarrow>
       ...
       \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> c2 sat [p \<inter> (-b),  R, G, q,a] \<Longrightarrow>      
       Sta p R \<Longrightarrow> (\<forall>s. (s,s) \<in> G) \<Longrightarrow>   
       \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> (Cond b c1 c2) sat [p, R, G, q,a]"
proof -  
 assume
    ...
    a4: "Sta p R" and
    a5: "(\<forall>s. (s,s) \<in> G)" 
  then have orig_rule:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (Cond b c1 c2) sat [p, R, G, q,a]" 
   using lrghoare.If by blast   
  { 
    fix s
    assume all_call:"\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]"
       ...
       { fix k ns ns'
         ...
         proof (cases "k=j")   
           case True          
             have "(s', s') \<in> G" 
               using a5  by blast
             thus "(snd (l ! k), snd (l ! Suc k)) \<in> (\<lambda>(pa, p). (Normal pa, Normal p)) ` G"
         ...
         next
           ...
           proof
             ...
             ultimately have drop_comm:"((\<Gamma>, drop (Suc j) l))\<in> comm(G, (q,a)) F"
               ...
               by blast                         
             show ?thesis 
               using l_k drop_comm a00 a21 a22 a10 \<Gamma>1 l_f  
...
proof -  
  assume
    ...
    a1:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (the (\<Gamma> f)) sat [p, R, G, q,a]" and
    a2:"\<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> (the (\<Gamma> f)) sat [p, R, G, q,a]" and        
    a3: "Sta p R" and
    a4: "(\<forall>s. (s,s) \<in> G)"
  then have orig_rule:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (Call f) sat [p, R, G, q,a]" 
    using lrghoare.Call[of f \<Gamma> \<Theta> F p R G q a] by blast     
         ...
         next
           ...
           proof -
             ...
             ultimately have drop_comm:"((\<Gamma>, drop (Suc j) l))\<in> comm(G, (q,a)) F"
               ...
               by blast                         
             then show ?thesis 
             using a00 a21 a22 a10 \<Gamma>1  j_k j_length l_f
...
lemma Seq_sound: 
      "\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c1 sat [p, R, G, q,a] \<Longrightarrow>
       ...
       \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> c2 sat [q, R, G, r,a] \<Longrightarrow>        
       Sta a R \<Longrightarrow>  (\<forall>s. (s,s) \<in> G) \<Longrightarrow>
       \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> (Seq c1 c2) sat [p, R, G, r,a]"
proof -  
  assume
    ...
    a4: "Sta a R" and 
    a5: "(\<forall>s. (s,s) \<in> G)"
  then have orig_rule:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (Seq c1 c2) sat [p, R, G, r,a]" 
    using lrghoare.Seq[of \<Gamma> \<Theta> F c1 p R G q a c2 r] by blast    
  { 
    fix s
    assume all_call:"\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]"
    ...
    {   
      ...
      proof - 
      {
       ...
       then have env_tran_right: "env_tran_right \<Gamma> l R" 
         using env_tran env_tran_right_def unfolding env_tran_def by auto       
       have "(\<forall>i ns ns'. Suc i<length l \<longrightarrow> 
               \<Gamma>\<turnstile>\<^sub>c(l!i)  \<rightarrow> (l!(Suc i)) \<longrightarrow>                 
               snd (l!i) = Normal ns \<and> snd (l!(Suc i)) = Normal ns' \<longrightarrow>              
                 (snd(l!i), snd(l!(Suc i))) \<in> 
                   (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G))\<and>
           ...
           next             
               ...
               assume a00:"Suc k<length l" and
                ...
                proof (cases "Suc k< ?lm_lc1")
                  ...
                  then have "(snd(?m_lc1!k), snd(?m_lc1!(Suc k))) \<in> 
                     (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)"
                  using a11' a22' m_lc1_comm True comm_dest1 l_f last_not_F by fastforce
             ...
             proof -
               ...
               proof -
                 ...
                 then show ?thesis
                   by (metis (no_types) Suc_eq_plus1 cp cp_lc1 final_always_env_i fst_conv less_diff_conv lm_lc1 snd_conv)
               qed                  
               ultimately have "snd (l ! (length l - 1)) \<in> Normal ` a"
                 using cp_lc1 a_normal a4 env_tran_right stability[of a R l ?lm_lc1 "(length l) -1" _ \<Gamma>] 
                 by fastforce
       ...
       next         
         ...
         have Gs2':"(s2', s2')\<in>G" using a5 by auto                     
         have concl:
         ...
         { fix k ns ns'
           assume a00:"Suc k<length l" and
            ...
            proof (cases "Suc k< i")
              ...
              then have "(snd(?m_lc1!k), snd(?m_lc1!(Suc k))) \<in> 
                 (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)"
              using a11' last_mcl1_not_F a22' m_lc1_comm True i_map length_c1_map 
                    comm_dest1[of \<Gamma>]
                by blast
              thus ?thesis using i_m_l by (simp add: a22 a22')  
            next
              ...
              { assume suck:"(Suc k=i)" 
                ...
                proof -
                  ...
                  moreover have "\<exists>p. p \<in> G \<and> (case p of (p, pa) \<Rightarrow> (Normal p, Normal pa)) = (Normal s2', Normal s2')"
                    by (meson  case_prod_conv mem_Collect_eq Gs2')
                  ultimately show ?thesis using suck k Normals2 by auto                      
                qed
              }
              next
              { 
                ...
                {assume "c2=Throw"
                 ...
                 using li Normals2 
                   by (metis a0  cp env_tran_right only_one_component_tran_j snd_conv)
                ...
                {assume "c2\<noteq>Throw"
                 ...
                 proof -
                     ...
                     using li lc2_l s2_normal lc2_not_empty by fastforce 
                   {
                     ...
                     ultimately have "(snd(lc2!j), snd(lc2!(Suc j))) \<in> 
                 (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (R)"
                 ...
                 moreover have 
                   "snd (lc2!k') = Normal ns \<and> snd (lc2!(Suc k')) = Normal ns'"                   
                  using k' suck' lc2_l a22 by fastforce
                 ultimately have "(snd (lc2! k'), snd (lc2 ! Suc k'))
                    \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` (G)"
                   using comm_lc2  lc2_last_f comm_dest1[of \<Gamma> lc2 G r a F k' ns ns'] 
                   by blast
         ...
         { assume final_l:"final (last l)" 
           ...
           then have final_lc2:"final (last lc2)" using final_l by auto
           {
             ...
             using li Normals2  env_tran_right cp c2_throw a0
             ...
             proof -
              have "\<forall>n. \<not> i \<le> n \<or> \<not> n < length l - 1 \<or> \<Gamma>\<turnstile>\<^sub>c l ! n \<rightarrow>\<^sub>e l ! Suc n"
                by (metis (no_types) Normals2 One_nat_def a0 add.right_neutral add_Suc_right c2_throw cp env_tran_right final_always_env_i less_diff_conv li snd_conv)
              then show ?thesis
                by meson
             qed               
             then have "snd (l!(length l - 1)) \<in> Normal ` (a) \<and> fst (l!(length l - 1)) = Throw"
               using a0 s2_a li a4 env_tran_right stability[of a R l i "(length l) -1" _ \<Gamma>] Throw
  ...
  {fix i ns ns'
   have "snd (last l1) \<notin> Fault ` F  \<longrightarrow>(Suc i < length (l1) \<longrightarrow>
        ...
        (SmallStepCon.final (last l1) \<longrightarrow>                  
                   ...
                   fst (last l1) = LanguageCon.com.Throw \<and>
                   snd (last l1) \<in> Normal ` a) "
      using comm_map''_catch[of \<Gamma> l1 c1 c2 s l2 G q a F i ns ns' r] a0 a1 a2 
      by fastforce
   }  then show ?thesis using comm_def unfolding comm_def by force       
qed
...
lemma Catch_sound: 
      "\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c1 sat [p,  R, G, q,r] \<Longrightarrow>
       ...
       \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> c2 sat [r,  R, G, q,a] \<Longrightarrow>        
       Sta q R \<Longrightarrow>  (\<forall>s. (s,s) \<in> G) \<Longrightarrow>
       \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> (Catch c1 c2) sat [p, R, G, q,a]"
proof -  
  assume
    ...
    a4: "Sta q R" and
    a5: "(\<forall>s. (s,s) \<in> G)"
  then have orig_rule:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (Catch c1 c2) sat [p, R, G, q,a]" 
    using lrghoare.Catch[of \<Gamma> \<Theta> F c1 p R G q r c2 a] by blast   
  { 
    fix s
    assume all_call:"\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]"
      ...
      proof - 
      {
                ...
                next
                  ...
                  proof
                    {assume suck:"(Suc k=?lm_lc1)"
                     ...
                     have G_s1':"(s1', s1')\<in> G" using a5 by auto
                     then show "(snd (l!k), snd (l!Suc k)) \<in>
                               (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)"
                     proof -
             ...
             proof -
               ...
               { fix nn :: nat
                 ...
                 then have "\<not> nn < length l - 1 \<or> \<not> length (map (lift_catch c2) lc1) \<le> nn \<or> \<Gamma>\<turnstile>\<^sub>c l ! nn \<rightarrow>\<^sub>e l ! Suc nn"
                   by (metis (no_types) One_nat_def add_Suc_right cp env_tran_right final_always_env_i fst_conv less_diff_conv lm_lc1) 
       ...
       next         
         ...
         then obtain lc1 lc2 where cp_lc1:"(\<Gamma>,lc1) \<in> (cp \<Gamma> c1 s) \<and> 
           ...
           using Catch_sound4 a0 using cp env_tran_right by blast           
         ...
         { fix k ns ns'
           assume a00:"Suc k<length l" and
            ...
            proof (cases "Suc k< i")
              ...
              then have "(snd(?m_lc1!k), snd(?m_lc1!(Suc k))) \<in> 
                 (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)"
              using a11' last_mcl1_not_F a22' m_lc1_comm True i_map length_c1_map comm_dest1[of \<Gamma>] 
                by blast
              thus ?thesis using i_m_l by (simp add: a22 a22')  
            next
              case False                                            
              ...
              { 
                ...
                {assume "c2=Skip"
                 then have "\<forall>k. k\<ge>i \<and> (Suc k < length l) \<longrightarrow> 
                            \<not>(\<Gamma>\<turnstile>\<^sub>c(l!k)  \<rightarrow> (l!(Suc k)))"
                 using li Normals2 
                   by (metis a0  cp env_tran_right only_one_component_tran_j)
                 then have ?thesis using a21 a001 k a00 by blast
                }  note left=this
                {assume "c2\<noteq>Skip"
                 ...
                 ultimately have "(snd (lc2! k'), snd (lc2 ! Suc k'))
                    \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` (G)"
                   using comm_lc2  lc2_last_f comm_dest1[of \<Gamma> lc2 G q a F k' ns ns'] 
                   by blast
                 then have ?thesis using suck' lc2_l k' by fastforce
                }                    
                then show ?thesis using left by auto                 
         ...
         { assume final_l:"final (last l)" 
           ...
           then have final_lc2:"final (last lc2)" using final_l by auto
           {
             ...
             proof -
              have "\<forall>n. \<not> i \<le> n \<or> \<not> n < length l - 1 \<or> \<Gamma>\<turnstile>\<^sub>c l ! n \<rightarrow>\<^sub>e l ! Suc n"
                using a0 c2_skip cp env_tran_right final_always_env_i by fastforce              
              then show ?thesis
                by meson
             qed               
...
proof -  
  assume
    ...
    a1:"\<forall>s. (s,s) \<in> G" and  
    a2: "(Sta p R) \<and> (Sta q R) \<and> (Sta a R)" 
  then have orig_rule:" \<Gamma>,\<Theta>\<turnstile>\<^bsub>/F\<^esub> (DynCom c1) sat [p, R, G, q,a]" 
    using lrghoare.DynCom[of p \<Gamma> \<Theta> F c1 R G q a] by blast            
  { 
    fix s
    ...
    {   
      ...
      {assume l_f:"snd (last l) \<notin> Fault ` F"       
       ...
       { fix k ns ns'
         ...
         proof (cases "k=j")   
           case True                                  
           have "(s', s')\<in>G" using a1 by fastforce
           thus "(snd (l ! k), snd (l ! Suc k)) \<in> (\<lambda>(pa, p). (Normal pa, Normal p)) ` G"
             using pair_j k_basic True ss ssj_normal_s by auto
         next
           case False   
           ...
           proof -
             ...
             then show ?thesis 
             using a00 a21 a22 a10 \<Gamma>1  j_k j_length l_f
             cptn_comm_induct[of \<Gamma> l "DynCom c1" s _ "Suc j" G q a F k ns ns']
             unfolding Satis_def by fastforce                         
          qed            
       qed 
       } thus ?thesis by (simp add: c_prod cp) qed
...
lemma Guard_sound:
  "\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c1 sat [p \<inter> g, R, G, q,a] \<Longrightarrow>
   \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> c1 sat [p \<inter> g, R, G, q,a] \<Longrightarrow>   
   Sta (p \<inter> g) R \<Longrightarrow> (\<forall>s. (s,s) \<in> G) \<Longrightarrow>
...
proof -  
  assume
    ...
    a3: "\<forall>s. (s,s) \<in> G"
  then have orig_rule:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (Guard f g c1) sat [p \<inter> g, R, G, q,a]" 
    using lrghoare.Guard[of \<Gamma> \<Theta> F c1 p g R G q a] by blast     
  { 
    fix s
    assume all_call:"\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]"  
    ...
    {   
      ...
      {assume l_f:"snd (last l) \<notin> Fault ` F"       
       ...
       { fix k ns ns'
         ...
         proof (cases "k=j")   
           case True                                  
           have " (s', s')\<in>G" using a3 by auto 
           thus "(snd (l ! k), snd (l ! Suc k)) \<in> (\<lambda>(pa, p). (Normal pa, Normal p)) ` G"
             using pair_j k_basic True ss ssj_normal_s by auto
         next
           case False   
           ...
           proof -
             ...
             then show ?thesis 
             using a00 a21 a22 a10 \<Gamma>1  j_k j_length l_f
             cptn_comm_induct[of \<Gamma> l "(Guard f g c1)" s _ "Suc j" G q a F k ns ns']
             unfolding Satis_def by fastforce                         
          qed            
       qed 
       } thus ?thesis by (simp add: c_prod cp) qed
...
proof -  
  assume
    ...
    a1:"\<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> c1 sat [p \<inter> g, R, G, q,a]" and      
    a2: "Sta p R" and
    a3: "(\<forall>s. (s,s) \<in> G)" and
    a4: "f\<in>F"
  then have orig_rule:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (Guard f g c1) sat [p, R, G, q,a]" 
    using lrghoare.Guarantee[of \<Gamma> \<Theta> F c1 p g R G q a] by blast   
  { 
    fix s
    assume all_call:"\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]"  
    then have a1:"\<Gamma> \<Turnstile>\<^bsub>/F\<^esub> c1 sat [p \<inter> g, R, G, q,a]" 
      ...
      {assume l_f:"snd (last l) \<notin> Fault ` F"       
       ...
       { fix k ns ns'
         ...
         {assume "s' \<in> (Collect (not (set_fun g)))"
          ...
          proof -
            have "j = k"
              by (metis (no_types) Suc_leI \<open>\<Gamma>\<turnstile>\<^sub>c (cj, sj) \<rightarrow> (csj, ssj) \<and> cj = fst (l ! j) \<and> sj = snd (l ! j) \<and> csj = fst (l ! Suc j) \<and> ssj = snd (l ! Suc j)\<close> \<open>csj = LanguageCon.com.Skip \<and> ssj = Fault f\<close> a00 a21 before_k_all_evnt cp env_tran_right le_eq_less_or_eq only_one_component_tran_j)
            then have False
              using \<open>\<Gamma>\<turnstile>\<^sub>c (cj, sj) \<rightarrow> (csj, ssj) \<and> cj = fst (l ! j) \<and> sj = snd (l ! j) \<and> csj = fst (l ! Suc j) \<and> ssj = snd (l ! Suc j)\<close> \<open>csj = LanguageCon.com.Skip \<and> ssj = Fault f\<close> a22 by auto
            then show ?thesis
              by metis
          qed
...
proof -
  ...
  proof- 
    ...
    thus ?thesis
      ...
      by fastforce
  qed    
  have G_ref:"(s1', s1')\<in>G" using a6 by fastforce
...
assume   
  ...
  then have G_ref:"(s1, s1) \<in> G"  using a5 by fastforce 
...
assume a00:"(\<Gamma>, (While b c, Normal s1) #
  ...
  then have G1_ref:"(sl, sl)\<in>G" using a8 by auto
  also have "snd (last ?sub_l) = Normal sl"
    using last_while1 last_while2 sl by fastforce
  ultimately have ?thesis 
    using a3 sl while_comm comm_while  comm_union[of \<Gamma> ?sub_l G p a F "(While b c, snd (last ((c, Normal s1) # xs1))) # ys"  _ a sl sl]   
    by fastforce 
  } note p1 =this
  {
...
assume a0:"(\<Gamma>, (c, Normal s1) # xs1) \<in> cptn_mod" and
  ...
  have step:"\<Gamma>\<turnstile>\<^sub>c(While b c,Normal s1) \<rightarrow> (Seq c (While b c),Normal s1)" using a2
    WhileTruec by fastforce
  have s1_normal:"s1\<in>p \<and> s1 \<in> b " using a6 a2 unfolding assum_def by fastforce
  then have G_ref:"(s1, s1)\<in>G" using a10 by auto
  have s1_collect_p: "Normal s1\<in> Normal ` (p \<inter> b)" using s1_normal by fastforce
  have "(\<Gamma>, map (lift (While b c)) ((c,Normal s1)#xs1))\<in>cptn" 
    using a2 cptn_eq_cptn_mod lift_is_cptn a0 by fastforce
  then have cptn_seq:"(\<Gamma>,(Seq c (While b c), Normal s1) # map (lift (While b c)) xs1) \<in>cptn" 
  ...
  then have G1_ref:"(sl, sl) \<in> G" using a10 by auto
  also have "snd (last ((While b c, Normal s1) # 
                  (Seq c (While b c), Normal s1) # 
                  map (lift (While b c)) xs1)) = Normal sl"
    using last_while1 last_while2 a3 by fastforce
  ultimately show ?thesis using throw_comm comm_while  comm_union[of \<Gamma> "((While b c, Normal s1) # 
                  (Seq c (While b c), Normal s1) # 
                  map (lift (While b c)) xs1)" _ _ _ _ "(LanguageCon.com.Throw, Normal sl) # ys"  _ _ sl sl]   
    by fastforce      
qed
...
lemma While_sound: 
      "\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> c1 sat [p \<inter> b, R, G, p,a] \<Longrightarrow>
       \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub> c1 sat [p \<inter> b, R, G, p,a] \<Longrightarrow>       
       Sta p R \<Longrightarrow>     
       Sta (p \<inter> (-b)) R \<Longrightarrow> Sta a R \<Longrightarrow> \<forall>s. (s,s) \<in> G \<Longrightarrow>
...
proof -  
  assume
    ...
    a4: "Sta a R" and
    a5: "\<forall>s. (s,s) \<in> G"
  then have orig_rule:"\<Gamma>,\<Theta> \<turnstile>\<^bsub>/F\<^esub> (While b c1) sat [p, R, G, p \<inter> (-b),a]" 
    using lrghoare.While[of \<Gamma>] by blast    
  { 
    fix s
    assume all_call:"\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]"  
...
lemma two': 
  ...
  \<Longrightarrow> \<forall>j i ns ns'. i<length clist \<and> Suc j<length l \<longrightarrow> 
      ...
      (snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Rely(xs!i)"
proof -
         ...
         a9: "snd (last l) \<notin> Fault ` F"
{
  assume a10:"\<exists>i j ns ns'. 
              ...
              \<not>(snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> 
                 (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Rely(xs!i)"
  then obtain j where 
    a10:"\<exists>i ns ns'. 
       i<length clist \<and> Suc j<length l \<and> 
       \<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>\<^sub>e  ((snd (clist!i))!Suc j) \<and>
       \<not>(snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> 
         (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Rely(xs!i)" by fastforce
   let ?P = "\<lambda>j. \<exists>i. i<length clist \<and> Suc j<length l \<and>
      \<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>\<^sub>e  ((snd (clist!i))!Suc j) \<and>       
      (\<not> (snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Rely(xs!i))"        
   obtain m where fist_occ:"(?P m) \<and> (\<forall>i<m. \<not> ?P i)" using exists_first_occ[of ?P j] a10 by blast
     ...
     then obtain i where
      ...
      (\<not> (snd((snd (clist!i))!m), snd((snd (clist!i))!Suc m)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Rely(xs!i))"
     by fastforce
    ...
    have compat:"(\<Gamma>\<turnstile>\<^sub>p(l!m)  \<rightarrow> (l!(Suc m))) \<and> 
     ...
     using a7 fst_occ unfolding conjoin_def compat_label_def by simp
     {
       assume a20: "(\<Gamma>\<turnstile>\<^sub>p(l!m)  \<rightarrow>\<^sub>e (l!(Suc m)) \<and> 
          (\<forall>i<length clist.  (fst (clist!i))\<turnstile>\<^sub>c (snd (clist!i))!m  \<rightarrow>\<^sub>e ((snd (clist!i))!(Suc m))))"
       then have "(snd (l!m),snd (l!(Suc m))) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` R"       
       using fst_occ a5  unfolding par_assum_def by fastforce
       then have "(snd(l!m), snd(l!(Suc m))) \<in> 
                       (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Rely(xs!i)"
     ...
     }note l = this
     {
      ...
      then obtain i' where "i'<length clist \<and>
               ((fst (clist!i'))\<turnstile>\<^sub>c ((snd (clist!i'))!m)  \<rightarrow> ((snd (clist!i'))!(Suc m)))"
      by auto
      then have "\<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub>  Com (xs ! i') sat [Pre (xs!i'), Rely (xs ! i'), Guar (xs ! i'), Post (xs ! i'),Abr (xs ! i')]"
...
lemma two: 
  ...
  \<Longrightarrow> \<forall>j i ns ns'. i<length clist \<and> Suc j<length l \<longrightarrow> 
      ...
      snd((snd (clist!i))!j) = Normal ns  \<and>  snd((snd (clist!i))!Suc j) = Normal ns' 
      \<longrightarrow> (snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Guar(xs!i) "
proof -
  assume a0:"\<forall>i<length xs. R \<union> (\<Union>j\<in>{j. j < length xs \<and> j \<noteq> i}. (Guar (xs ! j)))
       \<subseteq> (Rely (xs ! i))" and
         a1:"p \<subseteq> (\<Inter>i<length xs. (Pre (xs ! i)))" and
    ...
    \<Gamma>,\<Theta> \<Turnstile>\<^bsub>/F\<^esub>  Com (xs ! i) sat [Pre (xs!i), Rely (xs ! i), Guar (xs ! i), Post (xs ! i),Abr (xs ! i)]" and
         ...
         a9: "snd (last l) \<notin> Fault ` F"
  {
     assume a10:"(\<exists>i j ns ns'. i<length clist \<and> Suc j<length l \<and>  
      \<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>  ((snd (clist!i))!Suc j) \<and> 
      snd((snd (clist!i))!j) = Normal ns  \<and>  snd((snd (clist!i))!Suc j) = Normal ns' \<and>
       \<not> (snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Guar(xs!i)) "
     then obtain j where a10: "\<exists>i ns ns'. i<length clist \<and> Suc j<length l \<and>
      \<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>  ((snd (clist!i))!Suc j) \<and> 
      \<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>  ((snd (clist!i))!Suc j) \<and> snd((snd (clist!i))!j) = Normal ns  \<and>  snd((snd (clist!i))!Suc j) = Normal ns' \<and> 
      \<not> (snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Guar(xs!i)"
     by fastforce
     let ?P = "\<lambda>j. \<exists>i ns ns'. i<length clist \<and> Suc j<length l \<and>
      \<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>  ((snd (clist!i))!Suc j) \<and> 
      snd((snd (clist!i))!j) = Normal ns  \<and>  snd((snd (clist!i))!Suc j) = Normal ns' \<and>
      \<not> (snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Guar(xs!i)"     
     obtain m where fist_occ:"?P m \<and> (\<forall>i<m. \<not> ?P i)" using exists_first_occ[of ?P j] a10 by blast
     ...
     obtain i ns ns' where fst_occ:"i<length clist \<and> Suc m<length l \<and> 
      ...
      snd((snd (clist!i))!m) = Normal ns  \<and>  snd((snd (clist!i))!Suc m) = Normal ns' \<and>
      (\<not> (snd((snd (clist!i))!m), snd((snd (clist!i))!Suc m)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Guar(xs!i))"
     ...
     qed
     then have "(\<forall>ns ns'.
                      (Suc m < length (snd (clist ! i)) ) \<longrightarrow>
                      (\<Gamma>\<turnstile>\<^sub>c (snd (clist ! i)) ! m \<rightarrow> (snd (clist ! i)) ! Suc m) \<longrightarrow>
                      snd ((snd (clist ! i)) ! m) = Normal ns \<and>
                      snd ((snd (clist ! i)) ! Suc m) = Normal ns' \<longrightarrow>
                      (snd ((snd (clist ! i)) ! m), snd ((snd (clist ! i)) ! Suc m))
                      \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` Guar(xs!i))"
...
lemma three:
  "\<lbrakk>xs\<noteq>[]; \<forall>i<length xs. R \<union> (\<Union>j\<in>{j. j < length xs \<and> j \<noteq> i}. (Guar (xs ! j)))
   ...
   length xs=length clist; (\<Gamma>,l) \<in> par_cp \<Gamma> (ParallelCom xs) s; (\<Gamma>,l) \<in> par_assum(p, R) F;
    \<forall>i<length clist. clist!i\<in>cp \<Gamma> (Com(xs!i)) s; (\<Gamma>,l) \<propto> clist; (\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]);
    snd (last l) \<notin> Fault ` F\<rbrakk>
  \<Longrightarrow> \<forall>j i ns ns'. i<length clist \<and> Suc j<length l \<longrightarrow>  \<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>\<^sub>e  ((snd (clist!i))!Suc j) \<longrightarrow>
        snd((snd (clist!i))!j) = Normal ns \<and> snd((snd (clist!i))!Suc j) = Normal ns'
  \<longrightarrow> (snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> 
          (case_prod (\<lambda>x y. (Normal x, Normal y))) ` 
...
proof -
 assume a0:"xs\<noteq>[]" and
  ...
  assume a00:"i<length clist \<and> Suc j<length l" and
         a11: "\<Gamma>\<turnstile>\<^sub>c((snd (clist!i))!j) \<rightarrow>\<^sub>e  ((snd (clist!i))!Suc j)" and
         a12: "snd((snd (clist!i))!j) = Normal ns \<and> snd((snd (clist!i))!Suc j) = Normal ns'"
  ...
  proof        
    ...
    proof (cases "i'=i")
      ...
      proof (cases "S1 = S2")
        ...
        then have "(snd (l ! j), snd (l ! Suc j)) \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` (R)"
          using a00 all_e a6 unfolding par_assum_def by fastforce
    ...
    next
      ...
      then have clist_i'_Guardxs:"(snd((snd (clist!i'))!j), snd((snd (clist!i'))!Suc j)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Guar(xs!i')"
        using two a00 a12 False a8 unfolding conjoin_def same_state_def
        by (metis Suc_lessD a20 snd_conv) 
      ...
      by fastforce
      then have clist_i_Guardxs:"(snd((snd (clist!i))!j), snd((snd (clist!i))!Suc j)) \<in> 
                              (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Guar(xs!i')"  
...
lemma four:
  "\<lbrakk>xs\<noteq>[];  \<forall>i<length xs.  R \<union> (\<Union>j\<in>{j. j < length xs \<and> j \<noteq> i}. (Guar (xs ! j)))
   ...
   snd (l!i) = Normal ns \<and> snd (l!(Suc i)) = Normal ns'; (\<forall>(c,p,R,G,q,a)\<in> \<Theta>. \<Gamma> \<Turnstile>\<^bsub>/F\<^esub> (Call c) sat [p, R, G, q,a]);
   snd (last l) \<notin> Fault ` F\<rbrakk>
  \<Longrightarrow> (snd (l ! i), snd (l ! Suc i)) \<in> (case_prod (\<lambda>x y. (Normal x, Normal y))) `  (G)"
proof -
  assume a0:"xs\<noteq>[]" and
         a1:"\<forall>i<length xs.  R \<union> (\<Union>j\<in>{j. j < length xs \<and> j \<noteq> i}. (Guar (xs ! j)))
             \<subseteq>  (Rely (xs ! i))" and
         ...
         a9:"snd (l!i) = Normal ns \<and> snd (l!(Suc i)) = Normal ns'" and
   ...
   then have "(snd((snd (clist!j))!i) = Normal ns \<and>  snd((snd (clist!j))!Suc i) = Normal ns')"
     using a9 conjoin a7 unfolding conjoin_def same_state_def  by fastforce   
   also have 
     "\<forall>i j ns ns'. j < length clist \<and> Suc i < length l \<longrightarrow>
            \<Gamma>\<turnstile>\<^sub>c snd (clist ! j) ! i \<rightarrow> snd (clist ! j) ! Suc i \<longrightarrow>
             (snd((snd (clist!j))!i) = Normal ns \<and>  snd((snd (clist!j))!Suc i) = Normal ns') \<longrightarrow>
              (snd (snd (clist ! j) ! i), snd (snd (clist ! j) ! Suc i))
                \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` (Guar (xs ! j))"
    using two[OF a1 a3 a4 length_xs_clist a5 a6 clist_cp conjoin a10 a11] by auto
   ultimately have "(snd (snd (clist ! j) ! i), snd (snd (clist ! j) ! Suc i))
                \<in> (\<lambda>(x, y). (Normal x, Normal y)) ` Guar (xs ! j)"
     using a7 step_c length_Ps_clist step_clist by metis     
   then have "(snd (l!i), snd (l!(Suc i)))\<in> (\<lambda>(x, y). (Normal x, Normal y)) ` Guar (xs ! j)"
   ...
   { fix i
     ...
     proof-
       ...
       have r:"(\<forall>j. Suc j < length li \<longrightarrow> 
                    ...
                    (snd(li!j), snd(li!(Suc j))) \<in> 
                     (case_prod (\<lambda>x y. (Normal x, Normal y))) ` Rely (xs!i))"
         using i_length conjoin a1 length_xs_clist clist
              three[OF a0 a1 a2 a5 length_xs_clist a6 a7 clist_cp conjoin a10]        
         unfolding assum_def conjoin_def same_length_def Satis_def sep_conj_def by fastforce                                        
       ...
       { fix i ns ns'
         assume a20:"snd (last l) \<notin> Fault ` F"
         {
            assume a30:"Suc i<length l"  and
                   a31: "\<Gamma>\<turnstile>\<^sub>p(l!i)  \<rightarrow> (l!(Suc i))" and 
                   a32: "snd (l!i) = Normal ns \<and> snd (l!(Suc i)) = Normal ns'"
            ...
            qed            
            then have "(snd(l!i), snd(l!(Suc i))) \<in> 
                 (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)"
            using four[OF xs_not_empty a0 a1 a2 a5 c_par_cp c_par_assum a30 a31 a32 a00] by blast
            
         } then have "Suc i<length l \<longrightarrow> 
                     \<Gamma>\<turnstile>\<^sub>p(l!i)  \<rightarrow> (l!(Suc i)) \<longrightarrow> 
                     snd (l!i) = Normal ns \<and> snd (l!(Suc i)) = Normal ns' \<longrightarrow> 
                     (snd(l!i), snd(l!(Suc i))) \<in> 
                       (case_prod (\<lambda>x y. (Normal x, Normal y))) ` (G)  " by auto 
            note l = this
