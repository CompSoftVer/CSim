...
theory VcgCon 
imports  "../lib/VcgCommon" LocalRG_HoareDef 
...
syntax
  ...
  "_Assign_ev"      :: "'b => 'e \<Rightarrow>'b  \<Rightarrow> ('s,'p,'f,'e) com"    ("(_ :==(\<^sub>_)/ _)" [30,30, 30] 23)
  ...
  "_Init_ev"        :: "ident \<Rightarrow> 'c \<Rightarrow> 'e \<Rightarrow> 'b \<Rightarrow> ('s,'p,'f,'e) com" 
      ("(\<acute>_ :==\<^sub>_/ \<^bsub>_\<^esub>/ _)" [30,1000, 30] 23)
...
parse_translation {*
 ...
 [
  ...
  (@{syntax_const "_GuardedDynCallAss"}, Hoare_Syntax.call_ass_tr true true ev1 ev2 ev3 )] end
*}
